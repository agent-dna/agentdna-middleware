...
func Register(r *gin.Engine, h *handler.Handler) {
	...
	dashboard.GET("/agent-lhi-scores", h.AgentLHIScores)
	dashboard.GET("/tool-agent-scores", h.ToolAgentScores)
	dashboard.POST("/revoke-agent", h.RevokeAgent)
	dashboard.POST("/unrevoke-agent", h.UnrevokeAgent)
	...
	dashboard.GET("/threat-events", h.ThreatEvents)
	dashboard.GET("/top-threats", h.TopThreats)
	dashboard.GET("/threat-detail", h.ThreatDetail)
	dashboard.GET("/threat-by-id", h.ThreatByID)
